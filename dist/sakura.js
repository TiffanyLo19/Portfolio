...
var Sakura = function Sakura(selector, options) {
  ...
  this.createPetal = function () {
    ...
    var animationsArr = ["fall ".concat(fallTime, "s linear 0s 1"), "".concat(blowAnimation, " ").concat((fallTime > 60 ? fallTime : 60) - 20 + randomInt(0, 20), "s linear 0s infinite"), "".concat(swayAnimation, " ").concat(randomInt(1, 4), "s linear 0s infinite")];
    ...
    petal.style.borderRadius = "".concat(randomInt(_this.settings.maxSize, _this.settings.maxSize + Math.floor(Math.random() * 3)), "px ").concat(randomInt(1, Math.floor(width / 4)), "px");
    petal.style.height = "".concat(height, "px");
    petal.style.left = "".concat(Math.random() * document.documentElement.clientWidth - 10, "px");
    petal.style.marginTop = "".concat(-(Math.floor(Math.random() * 100) + 120), "px");
